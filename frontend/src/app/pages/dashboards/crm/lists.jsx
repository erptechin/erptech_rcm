...
const pageName = "CRM List"

export default function Lists({ tabData, customer }) {
  const { cardSkin } = useThemeContext();
  const [isOpen, { open, close }] = useDisclosure(false);
  const [autoResetPageIndex, skipAutoResetPageIndex] = useSkipper();
  const [orders, setOrders] = useState([]);

  const { data: info } = useInfo({ doctype: tabData?.doctype });
  const [search, setSearch] = useState({ doctype: tabData?.doctype, page: 1, page_length: 10, fields: null, filters: JSON.stringify([[tabData?.doctype, tabData?.filterName, "=", customer]]) });
  const { data } = useFeachData(search);

  useEffect(() => {
    if (info?.fields) {
      const fieldnames = info?.fields.filter(field => field.in_list_view).map(field => field.fieldname);
  ...
  const table = useReactTable({
    data: orders,
    columns: Columns(info?.fields.filter(field => field.in_list_view)),
    doctype: tabData?.doctype,
    state: {
      globalFilter,
      sorting,
      columnVisibility,
